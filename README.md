# WebStorm2019.06 破解方法分享

![WebStorm](https://upload-images.jianshu.io/upload_images/6206911-cacc0ddc0659833e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

## 前言

2017末初入前端行业，不巧 WebStorm 的注册码频频失效后，郁闷一段时间后, 网上查阅很多资料，终于找到稳定破解方法，持久更新到现在， 虽然现在主要用VScode，偶尔也会打开webstorm 用用。也深知做软件的不易，若资金允许，请点击 https://www.jetbrains.com/idea/buy/ 购买正版。

### 第一步：安装软件
（略）
### 第二步：复制补丁到软件安装目录
将`JetbrainsCrack-4.2-release.jar`文件安装目录bin文件下

### 第三步：修改安装目录下bin的两个文件`WebStorm.exe.vmoptions` 和`WebStorm64.exe.vmoptions`

在这两个文件末尾添加这行代码

-javaagent: 绝对路径、破解补丁名称（注意：补丁路径要正确，否则软件无法打开）

```
示例一：安装目录D:\Program Files\情况下
-javaagent:D:\Program Files\JetBrains\WebStorm\bin\JetbrainsCrack-4.2-release.jar

示例二：安装目录D:\sofeware\情况下
-javaagent:D:\sofeware\JetBrains\WebStorm\bin\JetbrainsCrack-4.2-release.jar
```
修改后，保存文件。确保补丁能够被读取到，破解才能生效。

### 第四步：启动WebStorm软件，选择activation code，复制下面代码进去。
```
56ZS5PQ1RF---
```

### 汉化

将resources_cn.jar文件复制到安装目录的lib目录下,软件会自动识别已有的语言，未汉化部分不受影响。

尝试数个汉化包,就目前这个包觉得兼容性最好，范围广，目前没有遇到Bug。有些引入造成有些功能直接打不开，或者无法使用。坑爹啊

### 美化主题

WebStorm右上角选择文件-->导入设置选项-->选择提供settings.jar -->导入重启即可
### [软件工具](https://pan.baidu.com/s/16Io9YQFrQjjTg1wqy6O3jg) 提取码: 2svu

> 欢迎关注个人微信公众号：前端之路那些事儿

![WeChat](https://i.loli.net/2019/10/08/GIHw3B7uPzOsyl4.png)


